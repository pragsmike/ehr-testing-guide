...
| A validator run offline reports clean output, but codes bound to LOINC/SNOMED CT/RxNorm are wrong | External codes are not validated without a terminology server — the gate's silence on those bindings was indeterminacy, and a binary pass/fail rounded it to pass | 25, 34 |
| Translations degrade only in the pipeline path that routes codes through an intermediate vocabulary | Chained crosswalks compose as relations: candidate sets multiply, side conditions conjoin, and somewhere a representative is being picked silently | 34 |
